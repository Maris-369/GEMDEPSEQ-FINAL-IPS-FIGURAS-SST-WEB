# Script completo para renombrar PDF y actualizar código
# Ejecutar desde la raíz del proyecto Gatsby

# 1. Detener el servidor Gatsby si está corriendo
Write-Host "Deteniendo servidor Gatsby si está corriendo..." -ForegroundColor Yellow
...
if (Test-Path $archivoOriginal) {
    Write-Host "✓ Archivo original encontrado" -ForegroundColor Green
} else {
    Write-Host "✗ No se encontró el archivo original en: $archivoOriginal" -ForegroundColor Red
    ...
    if ($archivosEncontrados) {
        ...
        Write-Host "No se encontró ningún archivo. Por favor, coloca el PDF en la carpeta del proyecto." -ForegroundColor Red
        exit 1
    }
}
...
if (!(Test-Path "static")) {
    ...
    Write-Host "✓ Carpeta static creada" -ForegroundColor Green
}

# 5. Copiar y renombrar el archivo
Copy-Item $archivoOriginal $rutaDestino -Force
Write-Host "✓ Archivo copiado a: $rutaDestino" -ForegroundColor Green
...
# Patrones a buscar
$patronesABuscar = @(
    "Estados Financieros 2024 2023 Supersalud y Policia 30MAYO - V2_copia.pdf",
    ...
    foreach ($archivo in $archivos) {
        ...
        if ($contenido) {
            ...
            if ($contenido -ne $contenidoOriginal) {
                ...
                Write-Host "✓ Actualizado: $($archivo.FullName)" -ForegroundColor Green
                $modificado = $true
            }
        }
    }
...
Write-Host "✓ Componente DownloadPDF creado en: $rutaComponente" -ForegroundColor Green

# 8. Mostrar resumen
Write-Host "`n=== RESUMEN DE CAMBIOS ===" -ForegroundColor Cyan
Write-Host "✓ PDF renombrado a: $nuevoNombre" -ForegroundColor Green
Write-Host "✓ PDF movido a: static/$nuevoNombre" -ForegroundColor Green
Write-Host "✓ Código actualizado automáticamente" -ForegroundColor Green
Write-Host "✓ Componente DownloadPDF creado" -ForegroundColor Green
...
Write-Host "3. Usar el componente DownloadPDF en tus páginas" -ForegroundColor Yellow
configurar-pdf.ps1
...
Write-Host "`n=== COMANDO DE VERIFICACIÓN ===" -ForegroundColor Cyan
